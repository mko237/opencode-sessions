...
|---|---|---|
| MGMT‑5 (Policy / Config) | Sets top‑level policy (system scope, recursion‑trigger threshold, child‑VSM config) that the lower‑level components read at startup. | VSM Wiki – “System 5” |
...
| OPS‑2 (Child VSM Monitor) | Tracks the health and performance of any spawned child VSMs, aggregates their metrics, and reports them back to MGMT‑5 for policy updates. | VSM Wiki – “System 2” |
...
**High‑level flow**
```
System 5 (Policy) ──► System 3 (Rule Engine) ──► System 1 (User I/O) (default)
      │                         │
...
|---|---|
| System 5 – Policy/Config | Holds global purpose and default System 1 type |
...
**End‑to‑end example**
1. System 5 defines `default_system1 = {"type":"user_io"}`.
...
##  Item 7
**System 2 – Information Channels & Resource Scheduler**

- Core function: provides a publish/subscribe API that any component (System 1, System 3, System 4, System 5) can use to emit and listen for events.
...
- Typical usage pattern:
  ...
  2. System 1 calls `request_resource(resource_id)` → System 2 checks the allocation table, reserves the slot, and returns a token.
  3. System 1 publishes its event; System 3 subscribes to the topic and, when needed, asks System 2 for the next free slot.
- Why it matters: without a central scheduler, each System 1 would need its own queue and conflict‑resolution logic; System 2 centralises both messaging and resource arbitration, keeping the design lightweight and extensible.

**System 5 – Policy / Configuration Source**
...
- Event emission: when a policy value changes (e.g., a new threshold is set or a limit is reached), System 5 publishes a `PolicyUpdate` event on System 2’s bus.
- Consumers:
...
- Interaction flow:
  1. System 5 updates its internal policy dict (`self.policy[key] = value`).
  2. System 5 calls `event_bus.publish("PolicyUpdate", {"key": key, "value": value})`.
  3. System 3 and System 4 receive the payload, extract the updated field, and apply it to their local state.
- Benefit: centralising policy in System 5 ensures a single source‑of‑truth; any component that needs the current policy can either read it directly from System 5 (synchronous) or react to the asynchronous `PolicyUpdate` events.
